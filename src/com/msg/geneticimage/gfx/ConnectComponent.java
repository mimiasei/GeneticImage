...
public class ConnectComponent
{
    ...
    /* Max difference between two pixels to be considered same label. */
    ...
    public int[] labeling(int[] image, Dimension d, boolean zeroAsBg)    
    {
        ...
        for (int y = 0; y < h; ++y ){
            for (int x = 0; x < w; ++x ){
                ...
                if (x > 0 && isSimilar(image, y*w+x-1, y*w+x)) {
                   k = rst[y*w+x-1];
                   connected = true;
                }
                // if connected to the up
                if (y > 0 && isSimilar(image, (y-1)*w+x, y*w+x) &&
                ...
                if ( x> 0 && isSimilar(image, y*w+x-1, y*w+x) && rst[y*w+x-1]!= k )
                        uf_union( k, rst[y*w+x-1], parent );
                if ( y> 0 && isSimilar(image, (y-1)*w+x, y*w+x) && rst[(y-1)*w+x]!= k )
    ...
    public boolean isSimilar(int[] pixels, int pixel, int neighbour) {
    	int blue = (int)(pixels[pixel] & 0xff); // blue
		int green = (int)((pixels[pixel] >> 8) & 0xff); // green
		int red = (int)((pixels[pixel] >> 16) & 0xff); // red
    	int nBlue = (int)(pixels[neighbour] & 0xff); // blue
		int nGreen = (int)((pixels[neighbour] >> 8) & 0xff); // green
		int nRed = (int)((pixels[neighbour] >> 16) & 0xff); // red
		int diff = Math.abs(blue - nBlue);
		diff += Math.abs(green - nGreen);
		diff += Math.abs(red - nRed);
