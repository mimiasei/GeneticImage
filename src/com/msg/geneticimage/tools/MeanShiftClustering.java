...
import com.msg.geneticimage.tools.KDTree.VectorDistanceTuple;
...
public final class MeanShiftClustering {
  ...
  public static List<DoubleVector> cluster(List<DoubleVector> points,
    ...
    List<DoubleVector> centers = observeCenters(kdTree, points, windowSize, verbose);
    ...
    for (int i = 0; i < maxIterations; i++) {
      ...
      if (verbose) {
        System.out.println("Iteration: " + i + " | Remaining centers converging: "
  ...
  private static double gaussianGradient(double stddev) {
    return -Math.exp(-(stddev * stddev) / 2d) / (SQRT_2_PI * stddev);
  }
